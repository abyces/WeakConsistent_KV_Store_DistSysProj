defmodule MerkleTree do
  ...
  defp convert_helper(clk) do
    Enum.map(clk, fn {k, v} -> Atom.to_string(k) <> Integer.to_string(v) end) |> Enum.join("")
  end

  defp convert_vals_to_string(vals) do
    Enum.map(vals, fn {v, clk} -> Integer.to_string(v) <> convert_helper(clk) end) |> Enum.join("")
  end
  ...
  def build(blocks, opts) do
    {hash_function, height, hash_leaves?, default_data_block} = get_from_options(opts, blocks)
    keys = Enum.map(blocks, fn {k, _} -> k end)
    values = Enum.map(blocks, fn {_, v} -> convert_vals_to_string(v) end)
    default_leaf_value = if hash_leaves?, do: hash_function.(default_data_block), else: default_data_block
    leaf_values = if hash_leaves?, do: Enum.map(values, hash_function), else: blocks
    default_leaf = %MerkleTree.Node{value: default_leaf_value, children: [], height: 0}
    leaves = Enum.map(Enum.zip(keys, leaf_values), fn {k, v} -> %MerkleTree.Node{value: v, children: [], height: 0, keys: [k]} end )
  ...
  defp _build(nodes, hash_function, height, default_leaf, final_height) do
    ...
    parents =
      Enum.map(children_partitions, fn partition ->
        ...
        keys = Enum.sort(List.flatten(Enum.map(partition, fn node -> Map.get(node, :keys) end)))
        %MerkleTree.Node{
